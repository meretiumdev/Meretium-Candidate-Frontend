...
export default function StrengthCard() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm font-manrope transition-all duration-300">
      <div className={`flex items-center justify-between ${isOpen ? 'mb-4' : 'mb-0'}`}>
          ...
          <div className="flex flex-col gap-4">
            {steps.map((step) => (
              <div key={step.title} className="flex items-center gap-3 group cursor-pointer">
                ...
                <span className={`text-[14px] ${step.completed ? 'text-[#475467]' : 'text-[#101828]'} ${step.completed && step.title !== 'Upload CV' ? 'line-through decoration-gray-300' : ''} font-medium`}>
                  {step.title}
                </span>
              </div>
            ))}
