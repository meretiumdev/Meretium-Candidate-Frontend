...
import { FileText, Trash2, Edit3, Upload, Star } from 'lucide-react';
...
export default function CVSection() {
  ...
  const [defaultCV, setDefaultCV] = useState('Frontend_Designer_CV.pdf');
  
  const cvs = [
    { name: 'Frontend_Designer_CV.pdf', date: 'Uploaded 2 days ago' },
    { name: 'Product_Designer_CV.pdf', date: 'Uploaded 1 week ago' },
    { name: 'General_CV.pdf', date: 'Uploaded 2 weeks ago' }
  ];
    ...
    <>
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mt-6 font-manrope transition-all duration-300">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-[18px] md:text-[20px] font-semibold text-[#101828]">My CVs</h2>
          <button 
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 text-[14px] font-medium text-[#475467] hover:text-[#101828] transition-colors cursor-pointer"
          >
          ...
          {cvs.map((cv) => (
            <div key={cv.name} className="p-4 bg-white border border-gray-200 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:bg-gray-50/50 hover:shadow-md cursor-pointer group">
               <div className="flex items-start sm:items-center gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                     <h3 className="text-[14px] md:text-[15px] font-medium text-[#101828] flex flex-wrap items-center gap-2">
                        ...
                        {defaultCV === cv.name && <span className="shrink-0 bg-[#D1FADF]/50 text-[#039855] text-[12px] font-medium px-2.5 py-0.5 rounded-full">Default</span>}
                     </h3>
               ...
               <div className="flex items-center justify-end gap-5 sm:gap-4 shrink-0 sm:ml-4">
                  <button 
                    onClick={(e) => { e.stopPropagation(); setDefaultCV(cv.name); }}
                    className={`transition-colors cursor-pointer ${defaultCV === cv.name ? 'text-[#FF6934]' : 'text-[#98A2B3] hover:text-gray-600'}`}
                  >
                    <Star size={18} fill={defaultCV === cv.name ? "#FF6934" : "none"} />
                  </button>
                  <button 
                    onClick={(e) => { e.stopPropagation(); setRenameTarget(cv.name); }}
                    className="text-[#98A2B3] hover:text-gray-600 transition-colors cursor-pointer"
                  >
                    <Edit3 size={18} />
                  </button>
                  <button 
                    onClick={(e) => { e.stopPropagation(); setDeleteTarget(cv.name); }}
