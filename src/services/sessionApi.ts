...
function getAuthSessionPath(sessionId: string): string {
  const normalizedBase = AUTH_API_BASE_URL.toLowerCase();
  const pathPrefix = normalizedBase.endsWith('/auth') ? '' : '/auth';
  return `${pathPrefix}/sessions/${encodeURIComponent(sessionId.trim())}`;
}
...
export async function deleteAuthSession(accessToken: string, sessionId: string): Promise<string | null> {
  ...
  const response = await executeAuthorizedRequest(trimmedAccessToken, (nextAccessToken) =>
    fetch(`${AUTH_API_BASE_URL}${getAuthSessionPath(trimmedSessionId)}`, {
      method: 'DELETE',
  ...
  if (code && /(ERROR|FAIL)/i.test(code)) {
    throw new Error(
      getApiDetailMessage(responsePayload)
      || getApiMessage(responsePayload)
      || `Session logout failed with code ${code}`
    );
  }
